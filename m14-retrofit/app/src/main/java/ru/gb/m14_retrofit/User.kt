...
data class User(
    ...
    val id: Id,
    val picture: Picture,
    val pictureComposite: String = picture.large,
    val nat: String
) {
    ...
    val registeredComposite: String
        get() = "${registered.date.take(10)} (${registered.age} years)"

    val allInfo: Map<String, String>
        get() = mapOf(
        "Gender" to gender,
        "Nationality"  to nat,
        "Date of Birth" to dobComposite,
        "e-mail" to email,
        "Phone" to phone,
        "Cell" to cell,
        "Location" to locationComposite,
        "Registered" to registeredComposite
    )
